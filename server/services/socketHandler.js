    ...
    io.on("connection", (socket) => {
        ...
        socket.on("sendMsg", async (data) => {
            try {
                ...
                if (receiverSocketId) {
                    // Send the message
                    io.to(receiverSocketId).emit("receiveMsg", message);
        ...
        socket.on("markAsRead", async (data) => {
            try {
                ...
                const result = await Chats.aggregate([
                    { $match: { chatUsers: receiverID, sender: { $ne: receiverID }, isRead: false } },
                    { $group: { _id: "$sender", count: { $sum: 1 } } },
                ]);
                const unreadMessageCount = result.length;
                ...
                if (receiverSocketId) {
                    io.to(receiverSocketId).emit("unreadMessageCount", unreadMessageCount);
                } else {
